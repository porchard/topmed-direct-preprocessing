...
# DIRECT = '/net/topmed11/working/porchard/direct-preprocessing/data/direct/Table-S7.txt'
...
direct['MAF'] = np.minimum(direct.A1_FREQ, direct.A2_FREQ)
direct = direct[['GeneID', 'SNPchr', 'Phenotype', 'SNPid', 'REF', 'ALT', 'MAF', 'SNPpos', 'Nominal_Pval', 'Coeff', 'AdjustedPval', 'DiscoveryOrder']]
...
to_lift = direct[['SNPchr', 'SNPpos', 'SNPid', 'REF', 'ALT']].drop_duplicates()
...
# update
# GeneID	SNPchr	Phenotype	SNPid	REF	ALT	MAF	SNPpos	Nominal_Pval	Coeff	AdjustedPval	DiscoveryOrder
direct_hg38 = direct[['GeneID', 'Phenotype', 'SNPid', 'MAF', 'Nominal_Pval', 'Coeff', 'AdjustedPval', 'DiscoveryOrder']]
direct_hg38 = direct_hg38[~direct_hg38.SNPid.isin(rejected.id)]
direct_hg38['Coeff'] = direct_hg38.Coeff * np.where(direct_hg38.SNPid.map(swapped_alleles), -1, 1)
